import PriceStats from '../models/PriceStats';
...
export class PriceAnalyticsService {
   ...
   */
  static async calculateAreaStats(
    country: string,
    city: string,
    area: string,
    ...
    const allPricesPerSqm: number[] = [];
    const typologyData = new Map<string, {
      prices: number[];
      surfaces: number[];
      pricesPerSqm: number[];
    }>();
    ...
    let stats = await PriceStats.findOne({ country, city, area, projectType });
    ...
    if (!stats) {
      ...
      stats.projectIds = projects.map(p => p._id) as any;
  ...
  static async getAreaStats(
    ...
    const query: any = { country, city, area, isActive: true };
    if (projectType) {
      query.projectType = projectType;
    }

    const stats = await PriceStats.find(query).sort({ projectType: 1 });
    return stats;
  }
  ...
  static async compareAreas(
    ...
    const stats = await PriceStats.find(query);
  ...
  static async getCityOverview(
    ...
    areas: Array<{
      area: string;
      villaAvg: number | null;
      immeubleAvg: number | null;
      sampleSize: number;
    }>;
    cityAverage: number;
    priceRange: { min: number; max: number };
  }> {
    const stats = await PriceStats.find({ country, city, isActive: true });
  ...
  static async searchByPriceRange(params: {
    ...
    if (params.minPricePerSqm || params.maxPricePerSqm) {
      projects = projects.filter(project => {
        ...
        const validTypologies = project.typologies.filter(
          (t: any) => t.price > 0 && t.surface > 0
        );
        if (validTypologies.length === 0) return false;

        const avgPricePerSqm = validTypologies.reduce(
          (sum: number, t: any) => sum + t.price / t.surface,
          0
        ) / validTypologies.length;
    ...
    const projectsWithPricePerSqm = projects.map(project => {
      ...
      const avgPricePerSqm = validTypologies.length > 0
        ? validTypologies.reduce((sum: number, t: any) => sum + t.price / t.surface, 0) / validTypologies.length
        : 0;
  ...
  static async getAreaTrends(
    ...
    currentAvg: number;
    history: Array<{ date: Date; avgPricePerSqm: number }>;
    trend: { direction: string; percentageChange: number };
  } | null> {
    const stats = await PriceStats.findOne({ country, city, area, projectType });
   ...
   */
  static async recalculateAllStats(): Promise<{
    updated: number;
    errors: number;
  }> {
    // Get all unique location/type combinations from projects
    const combinations = await Project.aggregate([
      {
        $match: {
          publicationStatus: 'published',
          status: { $nin: ['archive', 'suspended'] },
    ...
    for (const combo of combinations) {
      try {
        await this.calculateAreaStats(
          combo._id.country,
          combo._id.city,
          combo._id.area,
          combo._id.projectType
        );
        updated++;
      } catch (error) {
   ...
   */
  static async getAffordableAreas(
    country: string,
    city: string,
    projectType: 'villa' | 'immeuble',
    limit: number = 10
    ...
    const stats = await PriceStats.find({
      country,
      city,
      projectType,
      isActive: true,
