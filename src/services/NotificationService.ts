  ...
  static async create(params: {
    ...
    if (channels.email) {
      try {
        ...
        const user = await User.findById(params.recipient).select('email firstName');
        if (user?.email) {
          await sendEmail({
            to: user.email,
    ...
    if (channels.whatsapp) {
      try {
        const user = await User.findById(params.recipient).select('phone preferences');
        const phone = user?.phone;
        if (phone && user?.preferences?.notifications?.whatsapp !== false) {
          await WhatsAppService.sendTextMessage(phone, `${params.title}\n\n${params.message}`);
        }
      } catch (error) {
