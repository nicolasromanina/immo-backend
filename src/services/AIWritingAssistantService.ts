...
interface AIWritingRequest {
  type: 'project-description' | 'update-text' | 'lead-response' | 'announcement' | 'ad-copy';
  context: {
    projectName?: string;
    city?: string;
    price?: number;
...
export class AIWritingAssistantService {
   ...
   */
  static async generateText(request: AIWritingRequest): Promise<AIWritingResponse> {
   ...
   */
  static async generateWithAlternatives(
    request: AIWritingRequest,
    count = 3
  ): Promise<AIWritingResponse> {
    const results = await Promise.all(
      Array.from({ length: count }, () => this.generateText(request))
    );
   ...
   */
  static async improveText(text: string, instructions?: string): Promise<AIWritingResponse> {
    if (!this.GROQ_API_KEY) {
      throw new Error('GROQ_API_KEY is not configured');
    }
...
Réponds exclusivement avec la version améliorée du texte, sans ajout de commentaires, d’explications ou de métadonnées.`,
  ...
  static async generateUpdateText(data: {
    ...
    return this.generateText({
      ...
      context: {
        projectName: data.projectName,
        additionalContext: `Type de mise à jour: ${data.updateType}. ${
          data.progress ? `Avancement: ${data.progress}%` : ''
        } ${data.details || ''}`,
        tone: 'professional',
        language: 'fr',
      },
  ...
  static async generateLeadResponse(data: {
    ...
    return this.generateText({
      ...
      context: {
        projectName: data.projectName,
        additionalContext: `Nom du prospect: ${data.leadName}. ${
          data.inquiry ? `Demande: ${data.inquiry}` : ''
        }`,
        tone: 'professional',
        language: 'fr',
      },
  ...
  private static buildSystemPrompt(request: AIWritingRequest): string {
...
Réponds uniquement avec le texte demandé, sans commentaire ni explication.`;
  ...
  private static buildUserPrompt(request: AIWritingRequest): string {
