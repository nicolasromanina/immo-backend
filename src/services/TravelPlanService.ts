import TravelPlan, { ITravelPlan, ITravelVisit } from '../models/TravelPlan';
...
export class TravelPlanService {
  ...
  static async createPlan(data: {
    ...
    preferences?: Partial<ITravelPlan['preferences']>;
    notes?: string;
  }): Promise<ITravelPlan> {
    // Validate dates
  ...
  static async addVisit(
    ...
    const project = await Project.findById(visit.projectId).populate('promoteur');
    if (!project) {
    ...
    const newVisit: ITravelVisit = {
      project: visit.projectId as any,
      promoteur: project.promoteur as any,
  ...
  static async confirmVisit(
    ...
    visit.confirmedBy = confirmerId as any;
   ...
   */
  static async optimizeItinerary(planId: string, userId: string): Promise<ITravelPlan> {
    const plan = await TravelPlan.findById(planId).populate('visits.project');
    if (!plan) {
      throw new Error('Travel plan not found');
    }
    ...
    const visitsByDate = new Map<string, typeof confirmedVisits>();
    ...
    const optimizedItinerary: ITravelPlan['optimizedItinerary'] = [];

    for (const [dateStr, dayVisits] of visitsByDate) {
      // Sort by scheduled time
      dayVisits.sort((a, b) => 
        this.timeToMinutes(a.visit.scheduledTime) - this.timeToMinutes(b.visit.scheduledTime)
      );
   ...
   */
  static async generateShareLink(planId: string, userId: string): Promise<string> {
    const plan = await TravelPlan.findById(planId);
    if (!plan) {
      throw new Error('Travel plan not found');
    }
   ...
   */
  static async getPlanByShareToken(token: string): Promise<ITravelPlan | null> {
    return TravelPlan.findOne({ shareToken: token })
  ...
  static async sharePlan(
    planId: string,
    userId: string,
    shareWith: { email: string; name?: string }
  ): Promise<ITravelPlan> {
    const plan = await TravelPlan.findById(planId);
    if (!plan) {
      throw new Error('Travel plan not found');
   ...
   */
  static async getPromoteurUpcomingVisits(promoteurId: string): Promise<any[]> {
    ...
    for (const plan of plans) {
      for (const visit of plan.visits) {
        if (
          visit.promoteur.toString() === promoteurId &&
    ...
    return visits.sort((a, b) => 
      new Date(a.visit.scheduledDate).getTime() - new Date(b.visit.scheduledDate).getTime()
    );
  }
