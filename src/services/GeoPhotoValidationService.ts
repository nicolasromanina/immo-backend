...
import Update, { IGeolocatedPhoto } from '../models/Update';
...
export class GeoPhotoValidationService {
   ...
   */
  static async validatePhoto(
    photo: IGeolocatedPhoto,
    projectId: string
  ): Promise<{
    valid: boolean;
    score: number;
   ...
   */
  static async validateUpdatePhotos(
    updateId: string
  ): Promise<{
    ...
    const results = await Promise.all(
      update.geolocatedPhotos.map(async (photo) => {
   ...
   */
  static async processGeolocatedPhotos(
    updateId: string,
    photos: IGeolocatedPhoto[],
    userId: string
  ): Promise<IGeolocatedPhoto[]> {
    const update = await Update.findById(updateId);
    if (!update) {
      throw new Error('Update not found');
    }

    const processedPhotos: IGeolocatedPhoto[] = [];
    ...
    await AuditLogService.log({
      ...
      targetType: 'Update',
      targetId: updateId as any,
      metadata: {
        photosCount: photos.length,
        verifiedCount: processedPhotos.filter((p) => p.verified).length,
        averageScore: Math.round(
          processedPhotos.reduce((sum, p) => sum + (p.verificationScore || 0), 0) /
   ...
   */
  static async detectPhotoAnomalies(
    promoteurId: string
  ): Promise<{
    hasAnomalies: boolean;
    anomalies: Array<{
      type: 'repeated_image' | 'inconsistent_location' | 'suspicious_timing';
      description: string;
      updateIds: string[];
    }>;
  }> {
    ...
    const updatesByDay = new Map<string, string[]>();
    for (const update of recentUpdates) {
  ...
  static async generateVerificationReport(
    ...
    for (const update of updates) {
      if (update.geolocatedPhotos && update.geolocatedPhotos.length > 0) {
        ...
        const avgScore =
          update.geolocatedPhotos.reduce((sum, p) => sum + (p.verificationScore || 0), 0) /
