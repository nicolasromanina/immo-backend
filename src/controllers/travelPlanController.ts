...
export class TravelPlanController {
  ...
  static async createPlan(req: AuthRequest, res: Response) {
    try {
      ...
      const plan = await TravelPlanService.createPlan({
        userId: req.user!.id,
        tripName,
        destination,
   ...
   */
  static async getMyPlans(req: AuthRequest, res: Response) {
    try {
      const { status } = req.query;

      const plans = await TravelPlanService.getUserPlans(
        req.user!.id,
   ...
   */
  static async addVisit(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      ...
      const plan = await TravelPlanService.addVisit(id, req.user!.id, {
        projectId,
   ...
   */
  static async removeVisit(req: AuthRequest, res: Response) {
    try {
      const { id, visitIndex } = req.params;

      const plan = await TravelPlanService.removeVisit(
        id,
        parseInt(visitIndex),
        req.user!.id
      );
   ...
   */
  static async confirmVisit(req: AuthRequest, res: Response) {
    try {
      const { id, visitIndex } = req.params;
      const { contactPerson, contactPhone, meetingPoint } = req.body;

      const plan = await TravelPlanService.confirmVisit(
        id,
        parseInt(visitIndex),
        req.user!.id,
        { contactPerson, contactPhone, meetingPoint }
      );
   ...
   */
  static async completeVisit(req: AuthRequest, res: Response) {
    try {
      const { id, visitIndex } = req.params;
      ...
      const plan = await TravelPlanService.completeVisit(
        id,
        parseInt(visitIndex),
        req.user!.id,
        feedback
   ...
   */
  static async getSuggestedProjects(req: AuthRequest, res: Response) {
    try {
      ...
      const projects = await TravelPlanService.getSuggestedProjects({
   ...
   * Optimize itinerary
   */
  static async optimizeItinerary(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;

      const plan = await TravelPlanService.optimizeItinerary(id, req.user!.id);
   ...
   * Generate share link
   */
  static async generateShareLink(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;

      const shareLink = await TravelPlanService.generateShareLink(id, req.user!.id);
   ...
   */
  static async getSharedPlan(req: AuthRequest, res: Response) {
    try {
      const { token } = req.params;

      const plan = await TravelPlanService.getPlanByShareToken(token);
   ...
   */
  static async sharePlan(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      ...
      const plan = await TravelPlanService.sharePlan(id, req.user!.id, { email, name });
  ...
  static async updateStatus(req: AuthRequest, res: Response) {
    try {
      ...
      const plan = await TravelPlanService.updateStatus(id, req.user!.id, status);
   ...
   */
  static async getPromoteurUpcomingVisits(req: AuthRequest, res: Response) {
    try {
      ...
      const visits = await TravelPlanService.getPromoteurUpcomingVisits(promoteurId);
