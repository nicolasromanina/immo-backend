...
router.patch('/promoteurs/:id/founding-partner', (0, auth_1.authorizeRoles)(roles_1.Role.ADMIN), async (req, res) => {
    try {
        const { isFoundingPartner, discount, expiresAt } = req.body;
        const update = {};
        if (isFoundingPartner !== undefined)
            update.isFoundingPartner = isFoundingPartner;
        if (discount !== undefined)
            update.foundingPartnerDiscount = discount;
        if (expiresAt !== undefined)
            update.foundingPartnerExpiresAt = expiresAt ? new Date(expiresAt) : null;
        const Promoteur = (await Promise.resolve().then(() => __importStar(require('../models/Promoteur')))).default;
