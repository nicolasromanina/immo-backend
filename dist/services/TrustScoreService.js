...
const trustScore_1 = require("../config/trustScore");
class TrustScoreService {
    /**
     * Calculate trust score for a promoteur
     * Score is 0-100 based on multiple factors
     */
    static async calculatePromoteurTrustScore(promoteurId) {
        const promoteur = await Promoteur_1.default.findById(promoteurId).populate('badges.badgeId');
        if (!promoteur)
            return 0;
        let score = 0;
        const promoteurWeights = trustScore_1.trustScoreConfig.promoteur;
        ...
        const approvedFinancialDocs = promoteur.financialProofDocuments?.filter((d) => d.status === 'approved') || [];
        ...
        if (promoteur.averageResponseTime) {
            if (promoteur.averageResponseTime <= 2)
                score += promoteurWeights.responseMax;
            else if (promoteur.averageResponseTime <= 6)
                score += Math.round(promoteurWeights.responseMax * 0.7);
            else if (promoteur.averageResponseTime <= 24)
                score += Math.round(promoteurWeights.responseMax * 0.5);
            else if (promoteur.averageResponseTime <= 48)
                score += Math.round(promoteurWeights.responseMax * 0.2);
        }
        // 7. Badges (10 points)
        const badgeBonus = promoteur.badges.reduce((sum, b) => {
     ...
     */
    static async calculateProjectTrustScore(projectId) {
        const project = await Project_1.default.findById(projectId);
        if (!project)
            return 0;
        let score = 0;
        const projectWeights = trustScore_1.trustScoreConfig.project;
        ...
        if (project.media.coverImage)
            completeness += 3;
        if (project.media.renderings.length > 0)
            completeness += 3;
        if (project.media.photos.length > 0)
            completeness += 3;
        ...
        if (updates.length > 0) {
            ...
            if (daysSinceUpdate <= 14)
                score += Math.round(projectWeights.updatesMax * 0.66);
            else if (daysSinceUpdate <= 30)
                score += Math.round(projectWeights.updatesMax * 0.5);
            else if (daysSinceUpdate <= 60)
                score += Math.round(projectWeights.updatesMax * 0.33);
            else if (daysSinceUpdate <= 90)
                score += Math.round(projectWeights.updatesMax * 0.16);
        }
        ...
        if (project.faq.length > 0)
            score += Math.round(projectWeights.transparencyMax * 0.33);
        if (project.delays.length === 0)
            score += Math.round(projectWeights.transparencyMax * 0.33);
        else if (project.delays.length > 0 && project.delays[0].mitigationPlan)
            score += Math.round(projectWeights.transparencyMax * 0.2);
     ...
     */
    static async updateAllScores(promoteurId) {
        // Update promoteur score
        const promoteurScore = await this.calculatePromoteurTrustScore(promoteurId);
        await Promoteur_1.default.findByIdAndUpdate(promoteurId, { trustScore: promoteurScore });
        ...
        for (const project of projects) {
            ...
            await Project_1.default.findByIdAndUpdate(project._id, { trustScore: projectScore });
        }
     ...
     */
    static async getImprovementSuggestions(promoteurId) {
