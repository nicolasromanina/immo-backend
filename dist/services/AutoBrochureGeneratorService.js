...
exports.AutoBrochureGeneratorService = void 0;
const Project_1 = __importDefault(require("../models/Project"));
const Update_1 = __importDefault(require("../models/Update"));
const Document_1 = __importDefault(require("../models/Document"));
class AutoBrochureGeneratorService {
    /**
     * Generate a complete brochure for a project
     */
    static async generateBrochure(projectId, options = {}) {
        ...
        sections.push(this.generatePromoteurSection(project.promoteur, language));
        // 12. Contact section
        ...
        const hasImages = sections.some(s => s.type === 'gallery' || s.type === 'hero' ||
     ...
     */
    static generateHeader(project, language) {
        return {
            type: 'header',
            content: {
                projectTitle: project.title,
                ...
                promoteurLogo: project.promoteur?.logo || null,
            },
        };
    }
    /**
     ...
     */
    static generateHero(project) {
        return {
            type: 'hero',
            content: {
                coverImage: project.media?.coverImage || project.media?.renderings?.[0]?.url || project.media?.renderings?.[0] || null,
     ...
     */
    static generateDescription(project, language) {
        const titles = {
     ...
     */
    static generateLocation(project, language) {
        const titles = {
            fr: 'Localisation',
            en: 'Location',
        };
     ...
     */
    static generateTypologies(project, language) {
        const titles = {
     ...
     */
    static generateFeatures(project, language) {
        const titles = {
     ...
     */
    static generateTimeline(project, language) {
        const titles = {
     ...
     */
    static generateGallery(project) {
     ...
     */
    static async generateUpdates(projectId, language) {
        const titles = {
        ...
        return {
            ...
            content: {
                ...
                totalUpdates: await Update_1.default.countDocuments({ project: projectId, status: 'published' }),
            },
        };
    }
    /**
     ...
     */
    static async generateDocuments(projectId, language) {
        const titles = {
     ...
     */
    static generatePromoteurSection(promoteur, language) {
        const titles = {
        ...
        return {
            ...
            content: {
                ...
                badges: promoteur?.badges?.map((b) => b.badgeId?.name || b.name) || [],
     ...
     */
    static generateContact(project, language) {
        const titles = {
            fr: 'Nous contacter',
            en: 'Contact Us',
        };
     ...
     * Generate footer
     */
    static generateFooter(language) {
        const content = {
            fr: {
     ...
     */
    static extractHighlights(project, language) {
        const highlights = [];
     ...
     */
    static calculateWordCount(sections) {
        let count = 0;
     ...
     */
    static async generateSocialPost(projectId, platform, language = 'fr') {
        const project = await Project_1.default.findById(projectId)
        ...
        const coverImage = project.media?.coverImage ||
            project.media?.renderings?.[0]?.url ||
            project.media?.renderings?.[0] ||
            null;
        return {
            text,
            hashtags,
     ...
     */
    static async generateProjectSummary(projectId, maxLength = 200, language = 'fr') {
        const project = await Project_1.default.findById(projectId);
        if (!project) {
            throw new Error('Project not found');
        }
...
exports.AutoBrochureGeneratorService = AutoBrochureGeneratorService;
