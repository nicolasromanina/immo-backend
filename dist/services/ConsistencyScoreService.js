...
exports.ConsistencyScoreService = void 0;
const Project_1 = __importDefault(require("../models/Project"));
class ConsistencyScoreService {
    /**
     * Calculate consistency score for a project
     */
    static async calculateForProject(projectId) {
        const project = await Project_1.default.findById(projectId);
        if (!project)
            throw new Error('Projet non trouvé');
        const changesLog = project.changesLog || [];
        const flags = [];
        ...
        const overallScore = Math.round(priceStability * 0.30 +
        ...
        return {
            ...
            projectName: project.nom || project.title || project.name || 'Projet',
            overallScore,
            details: {
                priceStability,
                deliveryStability,
     ...
     */
    static evaluatePriceStability(changesLog, flags) {
        const priceChanges = changesLog.filter((c) => c.field === 'prix' || c.field === 'price' || c.field?.includes('prix'));
        if (priceChanges.length === 0)
            return 100;
        let score = 100;
        const sixMonthsAgo = new Date(Date.now() - 180 * 24 * 60 * 60 * 1000);
        const recentChanges = priceChanges.filter((c) => new Date(c.changedAt) > sixMonthsAgo);
     ...
     */
    static evaluateDeliveryStability(changesLog, flags) {
        const deliveryChanges = changesLog.filter((c) => c.field === 'dateLivraison' || c.field === 'deliveryDate' || c.field?.includes('livraison'));
        ...
        const pushbacks = deliveryChanges.filter((c) => {
     ...
     */
    static evaluateUpdateFrequency(changesLog, flags) {
        if (changesLog.length === 0) {
            flags.push('Aucune mise à jour enregistrée');
            return 30;
        }
        const now = new Date();
        const sortedChanges = [...changesLog].sort((a, b) => new Date(b.changedAt).getTime() - new Date(a.changedAt).getTime());
     ...
     */
    static evaluateCompleteness(project, flags) {
        let score = 0;
        const checks = [
     ...
     */
    static evaluatePhotoConsistency(changesLog, flags) {
        const imageChanges = changesLog.filter((c) => c.field === 'images' || c.field?.includes('photo') || c.field?.includes('image'));
        if (imageChanges.length === 0)
            return 80; // Neutral — no changes to evaluate
        let score = 100;
        ...
        for (const change of imageChanges) {
            if (change.oldValue)
                removedUrls.add(change.oldValue);
     ...
     */
    static async calculateForPromoteur(promoteurId) {
     ...
     */
    static async getFlaggedProjects(threshold = 50) {
        const projects = await Project_1.default.find({ status: { $ne: 'archived' } });
        const flagged = [];
        for (const project of projects) {
            try {
                const result = await this.calculateForProject(project._id.toString());
...
exports.ConsistencyScoreService = ConsistencyScoreService;
